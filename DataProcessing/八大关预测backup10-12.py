...
datat = pd.read_excel('7.30预测.xls').dropna()
...
origin_data = pd.DataFrame(datat)
...
model = lgb.Booster(model_file='lgb_first_attempt2_beizhai.txt')
...
merged_data_ori = pd.merge(pd.DataFrame(date_range111, columns=['日期']), origin_data, on='日期', how='outer')
merged_data_ori = pd.merge(pd.DataFrame(date_range111, columns=['日期']), merged_data_ori, on='日期', how='inner')

# print(date_range111)
# print(len(date_range111))
# print(origin_data['日期'][:2101])
print(merged_data)
merged_data_ori = merged_data_ori[:-1]
print(merged_data_ori)
true_value_list = merged_data_ori['O3VAL'].tolist()
print(true_value_list)
print(len(true_value_list))
