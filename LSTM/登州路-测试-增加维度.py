...
# 读取模型
model = tf.keras.models.load_model('登州路.keras')
print(model.summary())
...
data_values = data[data['C0007_PNAME'] == '登州路街道']
data_values = data_values.reset_index()
data_values = data_values.iloc[:, [3, 9]]
data_values = pd.DataFrame(np.insert(data_values.values, 879, values=[
                           pd.to_datetime('2024-06-14 15:00:00'), np.nan], axis=0))
...
for file in files:
    ...
    features = pd.concat([features.reset_index(), X_transformed_df.reset_index()], axis=1)
...
for result in result_list:
    ...
    y1 = ori_data[ori_data[0] >= beg_time]
    y2 = ori_data[ori_data[0] <= end_time]
    y = pd.merge(y1, y2, how='inner')
    y = y.to_numpy()

    plt.figure(figsize=(12, 6))
    plt.title("青岛市登州路街道 臭氧浓度 预测-真实比较(每1小时为单位)")
    plt.plot(result[:, 0], y[:, 1], label='True')
    plt.plot(result[:, 0], result[:, 1], label='Predictions')
    plt.xlabel('时间')
    plt.ylabel('臭氧浓度值')
    plt.show
...
# 输出到文件
with open('登州路-预测结果.txt', 'w') as f:

    for result in result_list:
        f.write(str(result[0, 0])[0:11])
        f.write("\n")
